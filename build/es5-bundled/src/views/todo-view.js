...
var vaadinControlStateMixin = {
  ControlStateMixin: ControlStateMixin
};
...
var vaadinDevelopmentModeDetector = {
  runIfDevelopmentMode: runIfDevelopmentMode
};
...
const ElementMixin$1 = superClass => class VaadinElementMixin extends superClass {
  /** @protected */static _finalizeClass() {
...
var vaadinElementMixin = {
  ElementMixin: ElementMixin$1
};
...
document.head.appendChild($_documentContainer.content); /* Only needed for IE11 when you want to use the dark palette inside the light palette */ /*
...
document.head.appendChild($_documentContainer$1.content); /* NOTICE: Generated with 'gulp icons' */ /*
                                                                                                    FIXME(polymer-modulizer): the above comments were extracted
                                                                                                    from HTML and may be out of place here. Review them and
                                                                                                    then delete this comment!
                                                                                                    */
...
$_documentContainer$4.innerHTML = `<dom-module id="lumo-field-button">
  <template>
    <style>
      [part\$="button"] {
        flex: none;
...
var vaadinThemePropertyMixin = {
  ThemePropertyMixin: ThemePropertyMixin
};
...
var vaadinThemableMixin = {
  ThemableMixin: ThemableMixin
};

class RadioButtonElement extends ElementMixin$1(ControlStateMixin(ThemableMixin(GestureEventListeners(PolymerElement)))) {
  static get template() {
    return html`
    <style>
      :host {
...
`;
  ...
  ready() {
    ...
    this.setAttribute('role', 'radiogroup'); // Need to check e.composed as a workaround for https://bugzilla.mozilla.org/show_bug.cgi?id=1472887
...
var vaadinRadioGroup = {
  RadioGroupElement: RadioGroupElement
};
const $_documentContainer$8 = html`<dom-module id="lumo-radio-button" theme-for="vaadin-radio-button">
  <template>
    <style>
      :host {
        -webkit-tap-highlight-color: transparent;
...
const $_documentContainer$9 = html`<dom-module id="lumo-radio-group" theme-for="vaadin-radio-group">
  <template>
...
const TextFieldMixin = subclass => class VaadinTextFieldMixin extends ControlStateMixin(subclass) {
  ...
  _addIEListeners(node) {
    /* istanbul ignore if */if (navigator.userAgent.match(/Trident/)) {
  ...
  attributeChangedCallback(prop, oldVal, newVal) {
    ...
    const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent); /* istanbul ignore if */
...
var vaadinTextFieldMixin = {
  TextFieldMixin: TextFieldMixin
};
...
class TodoView extends LitElement {
    ...
    return html$1` 
      ...
      <div class="input-layout"
        ...
        <vaadin-radio-group
          ...
          ${Object.values(VisibilityFilters).map(filter => html$1`
  ...
  buildTable(data) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      for (let p = 0; p < data[i].parallels.length; p++) {
        ...
        parallelsItems = html$1`${parallelsItems}
            <tr>
              <td>
  ...
  addParallelItems(collections) {
    ...
    for (const key of keys) {
      parallelsItemsTable = html$1`${parallelsItemsTable}
        <td>
  ...
  addParallels(inputArray) {
    ...
    inputArray.forEach(item => {
      parallelInput = html$1`${parallelInput}
