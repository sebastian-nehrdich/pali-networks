...
class SanskritTableView extends connect(store)(BaseView) {
    ...
    for (let i = 0; i < selectedParallels.length; i++) { 
      if (selectedParallels[i].probability <= this.probability) {
        ...
        selectedParallelsText = selectedParallelsText + 
              ...
              ${selectedParallels[i].parsegnr} ${selectedParallels[i].parsegmenttext}</span><br>
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let p = 0; p < data[i].parallels.length; p++) {
        if (data[i].parallels[p].probability <= this.probability && showCounter < this.maxResults) {      
          ...
          let segmentInputText = html`<a>${parSutta[0]} ${parSegmentName}</a>`;
      ...
      let segmentLink = html`<a>${data[i].segmentnr}</a>`;
...
customElements.define('sanskrit-table-view', SanskritTableView);
