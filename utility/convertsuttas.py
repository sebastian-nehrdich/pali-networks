...
fileWriteCsv = open(base_dir+'/pali-networks/utility/convertedcsv/edges.csv', 'w')
fileWriteCsv.write('Source,Target\n')

for name in os.listdir(suttadir):
		print(name)
		directory = re.findall(r'([a-z]*)', name)[0]
		try:
		    os.listdir(base_dir+'/pali-networks/suttas/'+directory)
		except:
		    os.mkdir(base_dir+'/pali-networks/suttas/'+directory)
		fileWrite = open(base_dir+'/pali-networks/suttas/'+directory+'/'+name[:-4]+'.json', 'w', encoding='utf-8')
		fileWrite.write('[')
...
with open(base_dir+'/pali-networks/utility/convertedcsv/edges.csv', newline='') as csvfile:
